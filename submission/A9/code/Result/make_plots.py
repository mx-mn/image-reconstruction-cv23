...
def main():
    ...
    plt.savefig('MSE_60_epochs.png')
    
    min_val_loss =  df[71:-1]['val_loss'].min()
    ax = df.plot(title=f'Training and Validation Loss \n best val loss:{min_val_loss:.4f}', marker='.', figsize=(16,9), alpha=0.6, xlabel='Epochs')
    plt.savefig('extra-training-with-mean-absolute-error.png')
    ...
    ax.set_yscale('log')
    plt.savefig('Last-epochs-plus-extra-training-with-mean-absolute-error.png')
